"""Generate plots for all 20 biomechanical features to visualize their distributions."""
...
def _draw_feature_boxplot(ax, df, feature_name, region, metric, colors, rng, show_ylabel=True, show_xlabel=True):
    """Helper to draw a single premium boxplot on a given axes."""
...
def plot_features_grid(df: pd.DataFrame) -> None:
    ...
    for row_idx, region in enumerate(REGIONS):
        for col_idx, metric in enumerate(METRICS):
            ...
            _draw_feature_boxplot(ax, df, feature_name, region, metric, colors, rng, show_ylabel, show_xlabel)
...
def plot_individual_features(df: pd.DataFrame) -> None:
    """Create and save an individual plot for each of the 20 features."""
    setup_matplotlib()
    ensure_dir(OUTPUT_DIR)
    ...
    for region in REGIONS:
        for metric in METRICS:
            ...
            _draw_feature_boxplot(ax, df, feature_name, region, metric, colors, rng, show_ylabel=True, show_xlabel=True)
            
            output_path = OUTPUT_DIR / f"{feature_name}_distribution.png"
            plt.savefig(output_path, dpi=150)
            plt.close(fig)
            print(f"[Success] Saved individual plot: {output_path.name}")
...
def main() -> None:
    ...
    print("Generating large 4x5 grid of features...")
    plot_features_grid(df)
    
    print("Generating 20 individual feature plots...")
    plot_individual_features(df)
